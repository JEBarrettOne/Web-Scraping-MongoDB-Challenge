...
def scrape():
    ...
    for i in range(0, len(click_links)):
        browser.links.find_by_partial_text(click_links[i]).click()
        
        html = browser.html
        soup = bs(html, 'html.parser')
        
        image = soup.find('img', class_='wide-image')['src']
        hemisphere = click_links[i]
    ...
    mars_info = {'hemisphere_image_urls': hemisphere_image_urls,'facts': mars_facts_url ,'featured_image': featured_image_url, 'news_title': news_title, 'news_desc': news_desc}
